...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.channel_id_entry.pack(pady=(0, 5), fill="x") # Reduced bottom padding

        # API Key Guidance Label
        ...
        # Settings Frame for Configuration Options
        ...
        # Analyze Selected Videos Button
    ...
    def start_fetch_videos_thread(self):
        ...
        self.analyze_button.configure(state="disabled") # Disable analyze during fetch too
        ...
        if not channel_id: # YouTube API key can be from .env via scraper
            self.log_queue.put("Error: Channel ID is required.\n")
            self.fetch_button.configure(state="normal"); self.analyze_button.configure(state="normal"); return
        for widget in self.video_list_frame.winfo_children(): widget.destroy()
        self.video_checkboxes = []; self.fetched_videos_data = []
    ...
    def _execute_fetch_videos(self, channel_id, yt_api_key, fw_api_key):
        ...
        if yt_api_key: command.extend(["--youtube-api-key", yt_api_key]) # Pass if provided
        
        max_scan = self.max_videos_scan_entry.get()
        try:
            max_scan_val = int(max_scan)
            if max_scan_val > 0: command.extend(["--max-videos-scan", str(max_scan_val)])
            else: self.log_queue.put("Warning: Invalid 'Max Videos to Scan'. Using script's default.\n")
        except ValueError: self.log_queue.put("Warning: 'Max Videos to Scan' not a valid number. Using script's default.\n")

        if fw_api_key: command.extend(["--fireworks-api-key", fw_api_key]) # Pass if provided
        ...
        try:
            ...
            stdout_data = "".join(iter(process.stdout.readline, '')) if process.stdout else ""
            stderr_data = "".join(iter(process.stderr.readline, '')) if process.stderr else ""
            if stderr_data: self.log_queue.put(f"Scraper Logs:\n{stderr_data}\n")
            ...
            if process.returncode == 0:
                ...
                try:
                    ...
                    if not isinstance(self.fetched_videos_data, list): self.fetched_videos_data = []
                except json.JSONDecodeError as e: self.log_queue.put(f"JSON Error: {e}\nStdout: {stdout_data[:500]}\n"); self.fetched_videos_data = []
            else: self.log_queue.put(f"Scraper failed (code {process.returncode}).\n"); self.fetched_videos_data = []
        except FileNotFoundError: self.log_queue.put("Error: scraper_v2.py not found.\n")
        except Exception as e: self.log_queue.put(f"Error running scraper: {str(e)}\n")
        finally: 
            self.log_queue.put("Fetch finished.\n")
            self.after(0, self._populate_gui_video_list) # Schedule GUI update on main thread
            self.fetch_button.configure(state="normal")
            self.analyze_button.configure(state="normal") # Re-enable analyze button

    def _populate_gui_video_list(self):
        for widget in self.video_list_frame.winfo_children(): widget.destroy()
        self.video_checkboxes = []
        if not self.fetched_videos_data:
            CTkLabel(self.video_list_frame, text="No videos fetched or error.").pack(pady=10); return
        self.log_queue.put(f"Populating GUI with {len(self.fetched_videos_data)} videos.\n")
        self.video_list_frame.update_idletasks(); frame_width = self.video_list_frame.winfo_width()
        label_wraplength = max(1, frame_width - 80)
        for video_data in self.fetched_videos_data:
            title = video_data.get('title', 'N/A'); video_id = video_data.get('video_id', 'N/A')
            if title == 'N/A' or video_id == 'N/A': continue
            item_frame = customtkinter.CTkFrame(self.video_list_frame)
            checkbox = customtkinter.CTkCheckBox(item_frame, text=""); checkbox.pack(side="left", padx=(2,5))
            self.video_checkboxes.append({"checkbox": checkbox, "video_id": video_id, "video_title": title, "url": video_data.get('url', ''), "description": video_data.get('description', ''), "reasoning_for_political": video_data.get('reasoning_for_political', '')})
            label = CTkLabel(item_frame, text=f"{title} (ID: {video_id})", anchor="w", wraplength=label_wraplength); label.pack(side="left", fill="x", expand=True, padx=(0,2))
            item_frame.pack(fill="x", pady=2, padx=2)
        if not self.video_checkboxes: CTkLabel(self.video_list_frame, text="No valid videos to display.").pack(pady=10)
    ...
    def start_analyze_videos_thread(self):
        ...
        self.fetch_button.configure(state="disabled"); self.analyze_button.configure(state="disabled")
        temp_input_file_path = "temp_selected_videos.json"
        try:
            with open(temp_input_file_path, 'w', encoding='utf-8') as f: json.dump(self.selected_videos_for_analysis_cache, f, indent=4)
    ...
    def _execute_analyze_videos(self, temp_input_file_path):
        try: os.makedirs(self.analysis_output_dir, exist_ok=True)
        except Exception as e: self.log_queue.put(f"Error creating output dir: {str(e)}\n"); self.after(0, self._finalize_analysis_ui); return
        
        command = ["python", "-u", "scraper_v2.py", "--action", "analyze_videos", "--input-file", temp_input_file_path, "--output-dir", self.analysis_output_dir]
        
        comments_count = self.comments_to_analyze_entry.get()
        if comments_count.lower() == 'all': command.extend(["--comments-to-analyze", "all"])
        else:
            try:
                comments_val = int(comments_count)
                if comments_val >= 0: command.extend(["--comments-to-analyze", str(comments_val)])
                else: self.log_queue.put("Warning: Invalid 'Comments to Analyze' value. Using script's default.\n")
            except ValueError: self.log_queue.put("Warning: 'Comments to Analyze' not a valid number or 'all'. Using script's default.\n")

        youtube_key = self.youtube_api_key_entry.get(); fireworks_key = self.fireworks_api_key_entry.get()
        if youtube_key: command.extend(["--youtube-api-key", youtube_key]) # Pass if provided
        if fireworks_key: command.extend(["--fireworks-api-key", fireworks_key]) # Pass if provided
        
        self.log_queue.put(f"Executing analysis: {' '.join(command)}\n")
        try:
            process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, bufsize=1, encoding='utf-8', errors='replace')
            # Log stderr progressively
            stderr_lines = []
            if process.stderr:
                for line in iter(process.stderr.readline, ''):
                    stderr_lines.append(line)
                    self.log_queue.put(f"Log: {line}") # Log each line of stderr as it comes
            
            process.wait() # Ensure process finishes
            # stdout_data = process.stdout.read() if process.stdout else "" # Read any remaining stdout
            # if stdout_data: self.log_queue.put(f"Analysis STDOUT:\n{stdout_data}\n")
            self.after(0, self._display_analysis_results) 
        except FileNotFoundError: self.log_queue.put("Error: scraper_v2.py not found.\n")
        except Exception as e: self.log_queue.put(f"Error running analysis: {str(e)}\n")
        finally:
    ...
    def _display_analysis_results(self):
        ...
        for video_data in self.selected_videos_for_analysis_cache: 
            video_title = video_data['title']; video_id = video_data['video_id']
            sanitized_title = self._sanitize_folder_name(video_title)
            ...
            def open_folder_action(folder_path=video_subfolder): 
                try:
            ...
            open_button = CTkButton(video_frame, text="Open Video Folder", command=open_folder_action)
            open_button.pack(anchor="e", padx=5, pady=2)
            files_to_check = ["contextual_summary.txt", "transcription.txt", "targeted_sentiment_analysis.json", "video_meta.json", "original_comments.json"]
        ...
        if not self.selected_videos_for_analysis_cache: 
            CTkLabel(self.individual_results_frame, text="No individual results to display.").pack(pady=10)
